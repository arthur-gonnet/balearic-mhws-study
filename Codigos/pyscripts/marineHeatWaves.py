...
def detect(t, temp, climatologyPeriod=[None,None], pctile=90, windowHalfWidth=5, smoothPercentile=True, smoothPercentileWidth=31, minDuration=5, joinAcrossGaps=True, maxGap=2, maxPadLength=False, coldSpells=False, alternateClimatology=False, Ly=False, cutMhwEventsByYear=False):
    ...
    mhw['severity_max'] = [] # [Category scale]
    mhw['severity_mean'] = [] # [Category scale]
    mhw['severity_var'] = [] # [Category scale]
    mhw['severity_cumulative'] = [] # [Category scale]
    ## END OF MODIFICATION
    ...
    ## MODIFICATION : Possibility to cut events to not mess with annual statistics
    ...
    for ev in range(mhw['n_events']):
        ...
        mhw['severity_max'].append(mhw_severity[tt_peak]) # [Category]
        mhw['severity_mean'].append(mhw_severity.mean()) # [Category]
        mhw['severity_var'].append(np.sqrt(mhw_severity.var())) # [Category]
        mhw['severity_cumulative'].append(mhw_severity.sum()) # [Severity Index.day]
        ## END OF MODIFICATION
...
def blockAverage(t, mhw, clim=None, blockLength=1, removeMissing=False, temp=None):
    ...
    mhwBlock['mean_thresh'] = np.zeros(nBlocks)
    ## END OF MODIFICATION
    ...
    #
    # Calculate block averages (MODIFIED FOR MEAN INTENSITY AND MEAN INTENSITY VAR.)
    #
    ...
    mean_threshold = (clim['thresh'] - clim['seas']).mean()
    for iBlock in range(nBlocks):
        mhwBlock['mean_thresh'][iBlock] = mean_threshold
        ## END OF MODIFICATION
    ...
    if removeMissing:
        ...
        for y in range(len(missingYears)):
            ...
            mhwBlock['mean_thresh'][iMissing] = np.nan
            ## END OF MODIFICATION
